...
(defn new-entry-view
  ...
  [:div.l-box-lrg.pure-g
   [:div.pure-u-1
    ...
    #_(h/pp-div @local)
    #_(let [tags (:tags (:entry @local))]
        (when (seq tags)
          [:div.pure-u-1.hashtags
           (for [hashtag tags]
             ^{:key (str "tag-" hashtag)}
             [:span.hashtag hashtag])]))
    [:div.entry-footer
      ...
      [:button.pure-button.button-xsmall
       {:on-click #(put-fn [:import/photos])} [:span.fa.fa-camera-retro] " import"]
      [:button.pure-button.button-xsmall
       {:on-click #(put-fn [:import/geo])} [:span.fa.fa-map-o] " import"]
      [:button.pure-button.button-xsmall
       {:on-click #(put-fn [:import/phone])} [:span.fa.fa-mobile-phone] " import"]]]])
