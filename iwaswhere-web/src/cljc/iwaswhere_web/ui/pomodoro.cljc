...
(defn pomodoro-stats
  ...
  (let [pomodoros (filter #(= :pomodoro (:entry-type %)) entries)
        ...
        interruptions-str (when (pos? interruptions)
                            (str ". Interruptions: " interruptions))]
...
(defn pomodoro-stats-str
  "Shows some information about the number of pomodoros created and completed on
   any given day, where completion is achieved when the :completed-time equals
   the planned duration :planned-dur.
   Also, the total time logged via pomodoros is shown."
  [entries]
  (let [{:keys [pomodoros completed-pomodoros total-time interruptions-str]}
        (pomodoro-stats entries)]
    (when (pos? pomodoros)
      (if (= pomodoros completed-pomodoros)
        (str "Pomodoros: " completed-pomodoros ", "
...
(defn pomodoro-stats-view
  "Shows some information about the number of pomodoros created and completed on
   ...
   Also, the total time logged via pomodoros is shown."
  [entries]
  (let [{:keys [pomodoros completed-pomodoros total-time interruptions]}
        (pomodoro-stats entries)
        incomplete-pomodoros (- pomodoros completed-pomodoros)]
    (when (pos? pomodoros)
      [:span
       (if (<= completed-pomodoros 3)
         (into [:span] (map (fn [_] [:span.fa.fa-clock-o.completed])
                            (range completed-pomodoros)))
         [:span
          [:span.fa.fa-clock-o.completed]
          [:span.completed-cnt completed-pomodoros]])
       (if (<= incomplete-pomodoros 3)
         (into [:span] (map (fn [_] [:span.fa.fa-clock-o.incomplete])
                            (range incomplete-pomodoros)))
         [:span
          [:span.fa.fa-clock-o.incomplete]
          [:span.incomplete-cnt incomplete-pomodoros]])
